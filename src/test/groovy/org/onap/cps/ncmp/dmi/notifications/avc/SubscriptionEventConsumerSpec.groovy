...
class SubscriptionEventConsumerSpec extends MessagingBaseSpec {
    ...
    def 'Sends subscription event response successfully.'() {
        given: 'an subscription event response'
            ...
            def subscriptionEventResponse = new SubscriptionEventResponse(subscriptionName: 'cm-subscription-001',
                clientId: 'SCO-9989752', dmiName: 'ncmpDmiPlugin', cmHandleIdToStatus: cmHandleIdToStatusMap)
    ...
    def 'Populate cm handle id to status map successfully.'() {
        ...
        then: 'cm handle id to status map populated as expected'
            def expectedMap = ['CmHandle1':'accepted','CmHandle2':'accepted']
            expectedMap == result
    }

    def 'Extract dmi name from event key successfully.'() {
        given: 'a simulated event key'
            def eventKey = 'SCO-9989752-cm-subscription-001-ncmpDmiPlugin'
        when: 'dmi name is extracted from event key'
            def result = objectUnderTest.extractDmiNameFromEventKey(eventKey)
        then: 'dmi name is obtained as expected'
            result == 'ncmpDmiPlugin'
    }
}