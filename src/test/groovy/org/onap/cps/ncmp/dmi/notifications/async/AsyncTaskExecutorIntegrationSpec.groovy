...
class AsyncTaskExecutorIntegrationSpec extends Specification {
    ...
    def 'Publish and Subscribe message - failure'() {
        given: 'a sample message and key'
           def objectUnderTest = new AsyncTaskExecutor(cpsAsyncRequestResponseEventProducerService)
            def exception = new HttpClientRequestException('some cm handle', 'Node not found', HttpStatus.BAD_REQUEST)
        objectUnderTest.publishAsyncFailureEvent('ncmp-async-m2m-1', '123', DataAccessRequest.OperationEnum.READ, exception)
        when: 'consumer has a subscription'
            consumer.subscribe(['ncmp-async-m2m-1'] as List<String>)
        then: 'no exception is thrown'
            noExceptionThrown()
        and: 'the topic is polled'
            def records = consumer.poll(Duration.ofMillis(500))
        then: 'poll returns a record'
            assert records.size() == 1
        and: 'the record received is the event sent'
            def record = records.iterator().next()
            assert record.eventContent.response-code == HttpStatus.BAD_REQUEST
    }
