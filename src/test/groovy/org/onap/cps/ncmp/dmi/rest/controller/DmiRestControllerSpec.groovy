...
import org.onap.cps.ncmp.dmi.notifications.async.DmiAsyncRequestResponseEventProducerService
...
class DmiRestControllerSpec extends Specification {
    ...
    @SpringBean
    DmiAsyncRequestResponseEventProducer cpsAsyncRequestResponseEventProducer = new DmiAsyncRequestResponseEventProducer(Mock(KafkaTemplate), "my-topic-999")

    @SpringBean
    DmiAsyncRequestResponseEventProducerService mockCpsAsyncRequestResponseEventProducerService = new DmiAsyncRequestResponseEventProducerService(cpsAsyncRequestResponseEventProducer)

    @SpringBean
    AsyncTaskExecutor asyncTaskExecutor = new AsyncTaskExecutor(mockCpsAsyncRequestResponseEventProducerService)
    ...
    def 'PassThrough Running Returns OK when topic is used for async'(){
        given: 'correct JsonData'
            def readPassthroughRunning = "${basePathV1}/ch/some-cmHandle/data/ds/ncmp-datastore:passthrough-running" +
                "?resourceIdentifier=some-resourceIdentifier&topic=my-topic-1"
    ...
    def 'PassThrough Running Returns NOK when topic is used for async'(){
        given: 'a request that will not find resourcidentidier'
            def readPassthroughRunning = "${basePathV1}/ch/some-cmHandle/data/ds/ncmp-datastore:passthrough-running" +
                "?resourceIdentifier=ietf-netconf-monitoring:netconf-state&options=(fields=schemas/schema/location)" +
                "&topic=my-topic-1"
        when: 'endpoint is invoked'
            def jsonData = TestUtils.getResourceFileContent('readData.json')
            def response = mvc.perform(
    ...
    def 'PassThrough Operational Returns OK when topic is used for async'(){
        given: 'correct JsonData'
            def readPassThroughOperational ="${basePathV1}/ch/some-cmHandle/data/ds/ncmp-datastore:passthrough-operational" +
                "?resourceIdentifier=some-resourceIdentifier&topic=my-topic-999"
        when: 'endpoint is invoked'
            def jsonData = TestUtils.getResourceFileContent('readData.json')
            def response = mvc.perform(
