...
class DmiRestControllerSpec extends Specification {
    ...
    def 'Get resource data for pass-through operational with bad request.'() {
        ...
        and: 'dmi service is not invoked'
            0 * mockDmiService.getResourceData('some-cmHandle',
                'parent/child',
                'application/json',
                '(fields=myfields,depth=5)',
                'content=all')
    ...
    def '#scenario using passthrough running.'() {
       ...
       then: 'response status is #expectedResponseStatus'
        ...
        where: 'given request body and data'
            scenario                              | requestBodyFile                 | operationEnum                                  || expectedResponseStatus
            'write data with Create operation'    | 'createDataWithNormalChar.json' | CREATE                                         || CREATED.value()
            'write data with Update operation'    | 'updateData.json'               | UPDATE                                         || OK.value()
    }
