...
class ForwardedEventConsumerServiceSpec extends Specification {
    ...
    def 'Consume event interrupted by an exception'() {
        given: 'a consumer'
            def mockConsumer = Mock(KafkaConsumer)
            def objectUnderTest = new ForwardedEventConsumerService(mockConsumer)
        and: 'producer throws time out exception'
            mockConsumer.poll(Duration.ofMillis(2000)) >> { throw new RuntimeException('Unable to poll') }
        when: 'the record is consumed'
            objectUnderTest.consume()
        then: 'an run time exception has been catch'
            noExceptionThrown()
    }
}
