...
class DmiServiceImplSpec extends Specification {
    ...
    def 'Write resource data for passthrough running with wrong data.'() {
    ...
    def 'Write resource data using passthrough for a cm handle with a internal error response from sdnc.'() {
        given: 'sdnc operation returns a internal server error response'
            mockSdncOperations.writeResourceDataPassthroughRunnng(_, _, _, _) >> new ResponseEntity<String>('response json', HttpStatus.INTERNAL_SERVER_ERROR)
        when: 'write resource data using passthrough from service method invoked'
            objectUnderTest.writeResourceDataPassthroughForCmHandle('some-cmHandle',
                    'some-resourceIdentifier', 'some-dataType', new Object())
        then: 'a dmi exception is thrown'
            thrown(DmiException.class)
