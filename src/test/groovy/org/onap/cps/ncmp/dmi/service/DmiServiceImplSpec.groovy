...
class DmiServiceImplSpec extends Specification {
    ...
    def moduleList = [new ModuleReference()] as LinkedList<ModuleReference>
    ...
    def 'Get module sources.'() {
        ...
        when: 'get module sources is invoked with the given cm handle and a module list'
            def response = objectUnderTest.getModuleSources('some-cmHandle', moduleList)
        then: 'the response contains the expected response body'
            response == 'some-response-body'
    }

    def 'Get module sources for a failed get module schema request.'() {
        ...
        when: 'get module sources is invoked with the given cm handle and a module list'
            objectUnderTest.getModuleSources('some-cmHandle', moduleList)
        then: 'ModulesNotFoundException is thrown'
            thrown(ModuleSourceNotFoundException)
    }
}
