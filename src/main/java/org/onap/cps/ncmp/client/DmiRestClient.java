...
import org.onap.cps.ncmp.model.CpsConfiguration;
...
public class DmiRestClient {
    ...
    public ResponseEntity<String> registerCmHandle(final String jsonData) {
        final var registerCmHandlesUri = buildCpsUrl("cps-ncmp/api/ncmp-dmi/v1/ch");
        final var headers = new HttpHeaders();
        headers.setBasicAuth(cpsConfiguration.getAuthUsername(), cpsConfiguration.getAuthPassword());
        headers.set("Content-type", "application/json");
        final var httpEntity = new HttpEntity<>(jsonData, headers);
    ...
    private String buildCpsUrl(final String registerCmHandlePath) {
        final var cpsBaseUrl = cpsConfiguration.getBaseUrl();
