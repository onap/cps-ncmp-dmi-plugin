...
public class DmiRestController implements DmiPluginApi, DmiPluginInternalApi {
    ...
    public ResponseEntity<Object> dataAccessPassthroughOperational(final String resourceIdentifier,
        ...
        if (isReadDataAccessRequest) {
            if (Strings.isBlank(topicParamInQuery)) {
                final String resourceDataAsJson = getResourceData(cmHandle, resourceIdentifier, optionsParamInQuery,
            ...
            asyncTaskExecutor.executeTaskAsynchronously(() ->
                            getResourceData(cmHandle, resourceIdentifier, optionsParamInQuery, DmiService
                                    .RESTCONF_CONTENT_PASSTHROUGH_OPERATIONAL_QUERY_PARAM), topicParamInQuery,
    ...
    /**
     * This method fetches the resource for given cm handle using pass through running. It filters the response on
     * the basis of options query parameters and returns response. It supports both read and write operation.
     *
     * @param resourceIdentifier    resource identifier to fetch data
     * @param cmHandle              cm handle identifier
     * @param dataAccessRequest     data Access Request
    ...
    private String getSdncResponse(final String resourceIdentifier, final String cmHandle,
        ...
        if (isReadOperation(dataAccessRequest)) {
            sdncResponse = getResourceData(cmHandle, resourceIdentifier, optionsParamInQuery,
                    DmiService.RESTCONF_CONTENT_PASSTHROUGH_RUNNING_QUERY_PARAM);
        } else {
            sdncResponse = writeAndGetResourceData(resourceIdentifier, cmHandle, dataAccessRequest);
        }
        return sdncResponse;
    }

    private String writeAndGetResourceData(final String resourceIdentifier, final String cmHandle,
                                           final DataAccessRequest dataAccessRequest) {
        return dmiService.writeData(
    ...
    private String getResourceData(final String cmHandle, final String resourceIdentifier,
                                   final String optionsParamInQuery, final String restConfContentParamInQuery) {
        return dmiService.getResourceData(cmHandle,
                resourceIdentifier,
                optionsParamInQuery,
                restConfContentParamInQuery);
    }
