...
public class AsyncTaskExecutor {
    ...
    public void executeTaskAsynchronously(final Supplier<String> taskSupplier,
        ...
        CompletableFuture.supplyAsync(taskSupplier::get)
            ...
            .whenCompleteAsync((resourceDataAsJson, error) -> {
                if (error == null) {
                    log.info("***** JKE ***** about to publish event {} :: {}", requestId, resourceDataAsJson);
                    publishEvent(topicName, requestId, resourceDataAsJson, "SUCCESS", "200");
                } else {
                    log.info("***** JKE ***** ERROR!!!!");
                    final String errorMessage = error.getMessage();
                    log.error(errorMessage);
                    publishEvent(topicName, requestId, resourceDataAsJson, "FAILURE", "404");
    ...
    private void publishEvent(final String topicName, final String requestId, final String resourceDataAsJson,
                              final String status, final String code) {
        log.info("***** JKE ***** about to publish event 2");
        ...
        log.info("***** JKE ***** cpsAsyncRequestResponseEvent {}", cpsAsyncRequestResponseEvent);
