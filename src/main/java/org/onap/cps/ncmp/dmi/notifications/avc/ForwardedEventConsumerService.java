...
public class ForwardedEventConsumerService {
    ...
    public void consume() {
        try {
            final ConsumerRecords<String, ForwardedEvent> consumerRecords = consumer.poll(Duration.ofMillis(2000));

            consumerRecords.forEach(it -> processRecord(it));
    ...
    private void processRecord(final ConsumerRecord<String, ForwardedEvent> record) {
