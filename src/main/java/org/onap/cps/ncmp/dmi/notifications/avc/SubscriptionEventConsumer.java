...
public class SubscriptionEventConsumer {
    ...
    public void consumeSubscriptionEvent(@Payload final SubscriptionEvent subscriptionEvent,
        ...
        log.info("Subscription for ClientID {} is received at {} by dmi plugin.", clientId, timeStampReceived);
    ...
    private Map<String, String> populateCmHandleIdToStatus(final Set<String> cmHandleIds) {
        ...
        cmHandleIds.forEach(item -> result.put(item,
                Math.round(Math.random()) == 0 ? SubscriptionEventResponseStatus.ACCEPTED.getResponseStatus() :
                        SubscriptionEventResponseStatus.DECLINED.getResponseStatus()));
        return result;
    }
}
