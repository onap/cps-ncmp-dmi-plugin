...
public class DmiServiceImpl implements DmiService {
    ...
    public ModuleSet getModulesForCmHandle(final String cmHandle) throws DmiException {
        ...
        if (responseEntity.getStatusCode() == HttpStatus.OK) {
            ...
            final ModuleSet moduleSet = createModuleSchemaRequest(responseBody);
            return moduleSet;
        } else {
    ...
    private ModuleSet createModuleSchemaRequest(final String responseBody) {
        final var moduleSchemas = convertModulesToNodeSchema(responseBody);
        final List<ModuleSetSchemas> moduleRequests = new ArrayList<>();
        for (final ModuleSchemaProperties schemaProperties : moduleSchemas.getSchemas().getSchema()) {
            moduleRequests.add(convertModuleSchemaToModuleRequest(schemaProperties));
        }
        final var moduleRequest = new ModuleSet();
        moduleRequest.setSchemas(moduleRequests);
        return moduleRequest;
    }

    private ModuleSetSchemas convertModuleSchemaToModuleRequest(final ModuleSchemaProperties moduleSchemaProperties) {
        final var moduleRequest = new ModuleSetSchemas();
        moduleRequest.setModuleName(moduleSchemaProperties.getIdentifier());
        moduleRequest.setNamespace(moduleSchemaProperties.getNamespace());
        moduleRequest.setRevision(moduleSchemaProperties.getVersion());
        return moduleRequest;
    }
