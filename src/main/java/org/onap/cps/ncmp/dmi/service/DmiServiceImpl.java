...
public class DmiServiceImpl implements DmiService {
    ...
    @Override
    public String getModulesForCmHandle(final String cmHandle) throws DmiException, ModulesNotFoundException {
        final ResponseEntity<String> responseEntity = sdncOperations.getModulesFromNode(cmHandle);
        if (responseEntity.getStatusCode() == HttpStatus.OK) {
            final String body = responseEntity.getBody();
            if (body == null || body.isEmpty()) {
                throw ModulesNotFoundException.createWith(cmHandle);
            }
