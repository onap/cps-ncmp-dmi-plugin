        ...
        if (!StringUtils.isEmpty(optionsParamInQuery)) {
            ...
            queryParams.setAll(
                    Arrays.stream(splitTempQueryByComma)
                            .map(queryParamPair -> {
                                final String[] splitKeyValueByEqualsSymbol = queryParamPair.split("=", 2);
                                return splitKeyValueByEqualsSymbol;
                            })
                            .collect(Collectors.toMap(
                                    queryParam -> queryParam[0], // Add query key at first place
                                    queryParam -> queryParam.length > 1
                                            ? queryParam[1] // Add query value at second place
                                            : StringUtils.EMPTY)));
        }
        return queryParams;
    }
