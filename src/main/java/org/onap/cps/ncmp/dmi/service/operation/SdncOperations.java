...
import org.onap.cps.ncmp.dmi.constants.DmiPluginConstants;
...
public class SdncOperations {
    ...
    public SdncOperations(final SdncProperties sdncProperties, final SdncRestconfClient sdncRestconfClient) {
        ...
        topologyUrlOperational = UriComponentsBuilder.fromUriString(
                        DmiPluginConstants.TOPOLOGY_URL_TEMPLATE_OPERATIONAL)
                .query(DmiPluginConstants.TOPOLOGY_ID)
                .buildAndExpand(this.sdncProperties.getTopologyId()).toUriString();
        topologyUrlData = UriComponentsBuilder.fromUriString(DmiPluginConstants.TOPOLOGY_URL_TEMPLATE_DATA)
                .query(DmiPluginConstants.TOPOLOGY_ID)
                .buildAndExpand(this.sdncProperties.getTopologyId()).toUriString();
    }
    ...
    private MultiValueMap<String, String> getQueryParamsAsMap(final String optionsParamInQuery,
        ...
        if (!StringUtils.isEmpty(optionsParamInQuery)) {
            final String tempQuery = stripParenthesisFromOptionsQuery(optionsParamInQuery)
                    + DmiPluginConstants.COMMA_DELIMITER + restConfContentQueryParam;
            queryParams.setAll(
                    Arrays.stream(tempQuery.split(DmiPluginConstants.COMMA_DELIMITER))
                            .map(queryParamPair -> queryParamPair.split(DmiPluginConstants.QUERY_SEPARATOR,
                                    DmiPluginConstants.QUERY_MAP_LIMIT))
                            .collect(Collectors.toMap(
                                    queryParam -> queryParam[DmiPluginConstants.QUERY_PARAM_NAME_INDEX],
                                    queryParam -> queryParam.length > DmiPluginConstants.QUERY_PARAM_LENGTH
                                            ? queryParam[DmiPluginConstants.QUERY_PARAM_VALUE_INDEX]
                                            : StringUtils.EMPTY)));
        }
        return queryParams;
    }
    ...
    private String prepareGetSchemaUrl(final String nodeId) {
        return addResource(addTopologyDataUrlwithNode(nodeId), DmiPluginConstants.GET_SCHEMA_URL);
    }
    ...
    private String prepareGetOperationSchemaUrl(final String nodeId) {
        return UriComponentsBuilder.fromUriString(topologyUrlOperational)
                .path(DmiPluginConstants.NODE_ID)
                .pathSegment(DmiPluginConstants.YANG_EXT_MOUNT)
                .path(DmiPluginConstants.GET_SCHEMA_SOURCES_URL)
                .path(DmiPluginConstants.PATH_SEPARATOR)
    ...
    private List<ModuleSchema> convertToModuleSchemas(final String modulesListAsJson) {
        try {
            return JsonPath.using(jsonPathConfiguration).parse(modulesListAsJson).read(
                    DmiPluginConstants.PATH_TO_MODULE_SCHEMAS, new TypeRef<>() {
                    });
