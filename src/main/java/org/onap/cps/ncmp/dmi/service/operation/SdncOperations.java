...
public class SdncOperations {
    ...
    private List<String> buildQueryParamList(final String optionsParamInQuery, final String restconfContentQueryParam) {
        ...
        getOptionsParamAsList(optionsParamInQuery, queryParamAsList);
        queryParamAsList.add(restconfContentQueryParam);
        return queryParamAsList;
    }

    private void getOptionsParamAsList(final String optionsParamInQuery, final List<String> queryParamAsList) {
        if (!StringUtils.isEmpty(optionsParamInQuery)) {
            //strip parenthesis '()' from optionsParamInQuery
            final String tempQuery = optionsParamInQuery.substring(1, optionsParamInQuery.length() - 1);
