...
public class YangModuleFactory {
    ...
    private void initializeModuleJsonStrings() {
        MODULE_SET_TAGS.forEach(tag -> {
            moduleReferencesJsonMap.put(tag, generateModuleJson(NUMBER_OF_MODULES_PER_MODULE_SET,
                this::createModuleReferencesJson));
            moduleResourcesJsonMap.put(tag, generateModuleJson(NUMBER_OF_MODULES_PER_MODULE_SET,
                this::createModuleResourcesJson));
        });

        // Initialize default tag
        moduleReferencesJsonMap.put(DEFAULT_TAG,
            generateModuleJson(NUMBER_OF_MODULES_NOT_IN_MODULE_SET, this::createModuleReferencesJson));
        moduleResourcesJsonMap.put(DEFAULT_TAG,
            generateModuleJson(NUMBER_OF_MODULES_NOT_IN_MODULE_SET, this::createModuleResourcesJson));
    }
    ...
    private String generateModuleJson(final int numberOfModules, final Function<Integer, String> moduleJsonGenerator) {
        return moduleJsonGenerator.apply(numberOfModules);
    }

    private String createModuleReferencesJson(final int numberOfModules) {
        final List<ModuleReference> moduleReferencesList = new ArrayList<>(numberOfModules);
        for (int i = 0; i < numberOfModules; i++) {
            moduleReferencesList.add(new ModuleReference("module" + i, generateModuleRevision()));
        }
        return serializeToJson(new ModuleReferences(moduleReferencesList), "ModuleReferences");
    }

    private String createModuleResourcesJson(final int numberOfModules) {
        final List<ModuleResource> moduleResourceList = new ArrayList<>(numberOfModules);
        for (int i = 0; i < numberOfModules; i++) {
            final String moduleName = "module" + i;
            final String revision = generateModuleRevision();
            final String yangSource = generateYangSource(moduleName, revision);
    ...
    private String serializeToJson(final Object objectToSerialize, final String objectType) {
        try {
            ...
            log.error(SERIALIZATION_ERROR, objectType, e.getMessage());
            return null;
        }
    }

    private String generateModuleRevision() {
        int randomDays = (int) (Math.random() * 365 * 10);
        return LocalDate.of(2010, 1, 1).plusDays(randomDays).toString();
    }
