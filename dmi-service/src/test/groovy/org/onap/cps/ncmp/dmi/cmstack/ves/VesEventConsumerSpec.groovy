...
class VesEventConsumerSpec extends MessagingBaseSpec {
    ...
    def 'Consume a VES event'() {
        ...
        then: 'the dmi service is registered correctly'
            1 * dmiService.registerCmHandles(['pynts-o-du-o1'])

    }

    def 'Consume create event for existing cm handle'() {
        given: 'the cm handle is already registered with NCMP'
    ...
    def 'Consume a VES message but cm handle registration results in exception'() {
        given: 'Exception occurs when registering cm handle with NCMP'
