...
public class VesEventConsumer {
     ...
     */
    @KafkaListener(topics = "#{@vesEventTopics.topics}",
    ...
    public void consumeVesEvent(final VesEventSchema vesEventSchema) {
        ...
        log.info("SourceName(device/cmHandleId) from the VES event is : {}", sourceName);
        try {
            dmiService.registerCmHandles(List.of(sourceName));
        } catch (final CmHandleRegistrationException cmHandleRegistrationException) {
            log.warn("Exception occurred while registering the device : {} , cause : {}", sourceName,
                    cmHandleRegistrationException);
        } catch (final Exception exception) {
            log.warn("Exception occurred with cause : {}", exception);
        }
    }
